...
def parse_args():
    ...
    # DQN hyperparameters
    parser.add_argument('--lr', type=float, default=0.005) # 1e-4
    parser.add_argument('--epsilon', type=float, default=0.9)
    parser.add_argument('--epsilon_min', type=float, default=0.05)
    parser.add_argument('--gamma', type=float, default=0.99)
    parser.add_argument('--batch_size', type=int, default=64)
    parser.add_argument('--warmup_steps', type=int, default=1000) # 5000
    ...
    parser.add_argument('--max_steps', type=int, default=int(1e5)) # 2.5e5
    parser.add_argument('--eval_interval', type=int, default=2000) # 10000
    # others
...
def validation(agent, num_evals=5):
    ...
    for i in range(num_evals):
        ...
        while not done:
            ...
            action = agent.act(state, training=False)  # Choose action based on the current policy (without exploration)
            
            # get your action feedback from environment
            # next_state, reward, terminated, truncated, info = YOUR_CODE_HERE
            next_state, reward, terminated, truncated, info = eval_env.step(action)  # Execute the action in the environment
