...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict = {
        ...
        OBS0: [i for i in range((X_RANGE - 1) + (Y_RANGE - 1) + MAX_NOISE + 1)],
        OBS1: [i for i in range((X_RANGE - 1) + (Y_RANGE - 1) + MAX_NOISE + 1)]
    }
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        leftFactors = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for var in eliminationOrder:
            leftFactors, joinedFactors = joinFactorsByVariable(leftFactors, var)
            length = len(joinedFactors.unconditionedVariables())
            if length > 1: # If the factor has more than one unconditioned variable, eliminate the variable from the factor
                leftFactors.append(eliminate(joinedFactors, var))
        return normalize(joinFactors(leftFactors))
        raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        if total and total != 0:
            for key in list(self.keys()):  # Create a list of keys to safely iterate
                self[key] /= total
        else:
            return      # do nothing
        # raiseNotDefined()
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        elif noisyDistance is None: # if the ghost is not in jail and the distance is None, return 0 because in this case,  the ghost cannot be at the noisyDistance from Pacman. 
            return 0.0
        
        trueDistance = manhattanDistance(pacmanPosition, ghostPosition) # Calculate the true distance
        
        return busters.getObservationProbability(noisyDistance, trueDistance) # Return the probability of the noisy distance given the true distance
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        beliefs = self.beliefs # First get the initial belief of the pacman, as well as the pacman position and jail position

        pacmanPosition = gameState.getPacmanPosition()
        jailPosition = self.getJailPosition()
        for ghostPosition in self.allPositions:
            beliefs[ghostPosition] = self.getObservationProb(observation, pacmanPosition, ghostPosition, jailPosition) * self.beliefs[ghostPosition]
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        # Enumerate over all old positions
        for oldPos in self.allPositions: 
            newPosDist = self.getPositionDistribution(gameState, oldPos) # Get the distribution over new positions given the old position
            for newPos, prob in newPosDist.items(): # Update new beliefs based on the distribution from oldPos
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        # Get the legal positions if there are any. If not, do nothing and return.
        numPositions = len(self.legalPositions)
        if len(self.legalPositions) == 0:
            return  # Avoid division by zero if no legal positions

        # Initialize the particles by evenly distributing them across the legal positions
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # initialize the belief distribution as a DiscreteDistribution object
        ...
        # Normalize the belief distribution to convert the counts to probabilities
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Check if all weights are zero, if so, use the initializeUniformly function to reinitialize the particles. 
        if weights.total() == 0:
            self.initializeUniformly(gameState)
        # If not, resample the particles based on the weights
        else:
    ...
    def elapseTime(self, gameState):
        ...
        # Enumerate over all old particles 
        for oldPos in self.particles:
            # Get the distribution over new positions for this particle
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            # Sample a new position by randomly selecting from the distribution of new positions
