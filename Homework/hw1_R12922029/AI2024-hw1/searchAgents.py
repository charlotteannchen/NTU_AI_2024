...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        self.startingState = (self.startingPosition, tuple(cornerList))
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                cornerList = list(state[1]) # deep copy
                ...
                cost = 1
                successors.append( ( (nextState, tuple(cornerList)), action, cost, ) )
...
def findClosestPoint(location, goalArray):
    """
    Helper function for Corner Heuristic Algorithm
    """
...
def mazeDistance(point1, point2, gameState):
    ...
    assert not walls[xy1[0]][xy1[1]], 'point1 must be a wall: ' + str(point1)
    assert not walls[xy2[0]][xy2[1]], 'point2 must be a wall: ' + str(point2)
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for i in range(len(cornersUnvisited)):
        if not cornersUnvisited[i]:
            unexploredSet.append(corners[i])

    #calculate the distance from current node to all corner nodes
    if len(unexploredSet) > 0:
        closestPoint = findClosestPoint(currLocation, unexploredSet)
        farthestPoint = findFarthestPoint(currLocation, unexploredSet)

        currNode = problem.startingGameState
        closestNode = unexploredSet[closestPoint[0]]
        farthestNode = unexploredSet[farthestPoint[0]]

        currToClosest = mazeDistance(currLocation, closestNode, currNode)
        closestToFarthest = mazeDistance(closestNode, farthestNode, currNode)

        heuristic = currToClosest + closestToFarthest
