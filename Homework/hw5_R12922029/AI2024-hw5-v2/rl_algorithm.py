...
class DQN:
    ...
    def learn(self):
        ...
        action = action.to(self.device).long()  # Ensuring action is of the correct type and on the correct device
        reward = reward.to(self.device)
        next_state = next_state.to(self.device)
        terminated = terminated.to(self.device)

        q_values = self.network(state)  # This should output [batch_size, num_actions]
        action_indices = action.unsqueeze(-1)  # Reshape action to [batch_size, 1] for gathering

        print("Unexpected tensor dimensions. q_values: {}, action_indices: {}".format(q_values.shape, action_indices.shape))

        # Gather the specific Q-values for the chosen actions
        ...
        td_target = reward + (1 - terminated) * self.gamma * next_q
    ...
    def process(self, transition):
        ...
        # self.epsilon_decay = (epsilon - epsilon_min) / 1e6
        self.epsilon_min = self.epsilon - self.epsilon_decay * 1e6 
