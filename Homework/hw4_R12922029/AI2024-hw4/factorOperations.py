...
def joinFactors(factors: List[Factor]):
    ...
    for factor in factors:
        ...
        conditioned.difference_update(unconditioned_vars) 
        conditioned_vars.update(conditioned) 
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        new_unconditioned_vars = unconditioned_vars - {eliminationVariable}
        new_factor = Factor(new_unconditioned_vars, conditioned_vars, variable_domains)
        for new_assignment in new_factor.getAllPossibleAssignmentDicts():
            total_probability = 0
            for elim_value in variable_domains[eliminationVariable]:
