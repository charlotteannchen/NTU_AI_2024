...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # print(f"FactorsList:{FactorsList}")
        for var in eliminationOrder:
            FactorsList, joinedFactor = joinFactorsByVariable(FactorsList, var)
            # print(f"for FactorsList:{FactorsList}")
            # print(f"joinedFactor:{joinedFactor}")
            length = len(joinedFactor.unconditionedVariables())
            if  length > 1:
            # if var in joinedFactor.unconditionedVariables():
                FactorsList.append(eliminate(joinedFactor, var))
            #     print(f"eliminate(joinedFactor, var)")
            # print(f"end for FactorsList:{FactorsList}")
        
        joined = joinFactors(FactorsList)
        # print(f"joined:{joined}")
        # print(f"normalize:{normalize(joined)}")
        return normalize(joined) # Let the sum of probablity be 1
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        # raiseNotDefined()
        discreteDistribution = self.beliefs

        for position in self.allPositions:
            discreteDistribution[position] *= self.getObservationProb(observation, gameState.getPacmanPosition(), position, self.getJailPosition())
            # discreteDistribution[position] = discreteDistribution[position] * prob
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        loop = int(numPart / len(LegalPos))
        while loop:
            loop = loop - 1
            uniform += LegalPos
        remain = numPart % len(LegalPos)
        uniform += LegalPos[0: remain]
